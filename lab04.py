...
def buildDiscriminator():
    ...
    model.add(Conv2D(512, (5, 5), strides=(2, 2), padding='same', input_shape=[28, 28, 1]))
...
def buildGAN(images, epochs = 40000, batchSize = 32, loggingInterval = 0):
    ...
    for epoch in range(epochs):
        ...
        if loggingInterval > 0 and epoch % loggingInterval == 0:
            ...
            plot_graph(gen_loss, dis_loss, OUTPUT_DIR + "/"
                       + "Loss Graphs" +"/" + OUTPUT_NAME + str(epoch) + "loss_plot.png")
...
# Plot graphs
def plot_graph(glist, dlist, path):
    y_gen = glist
    y_dis = dlist
    X = range(len(y_dis))

    plt.plot(X, y_dis, label='Discriminator')
